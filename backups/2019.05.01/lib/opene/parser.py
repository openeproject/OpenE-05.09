            ...
            if (variable_value.type in ["sum", "integer", "float"]):
                ...
                for row_number, row_content in enumerate(variable_value.value):
                    ...
                    for value in row_content:
                        value_to_return += self.value_of(value) + "\t"
                    value_to_return += "\n"
                return value_to_return
        except KeyError:
            return False
                    ...
                    if (len(tokens) == 1):
                        if (tokens[i].type == "command"):
                            if (tokens[i].value in DataManager.classes):
                                ...
                                if (variable_value):
                                    print("{} <- {}".format(tokens[i].value, self.debug_variable(tokens[i].value)))
                                else:
                                    errors.raise_error("UndefinedVariable",
                                                       tokens[i].line,
                                                       tokens[i].col,
        ...
        if (function_name in self.functions):
            ...
            if (len(DataManager.external_files) > 0):
                ...
                for external_file in DataManager.external_files:
                    if (not ran_external):
                        function_out = external_file.ExternalParser().parse(function_name=function_name,
                                                                            parameters=values,
                                                                            errors=errors)
                        if (function_out["ran"]):
                            ran_external = True
                ...
                if (value.type == "string"):
                    ...
                    try:
                        ...
                        if (hasattr(imported_module, "ExternalParser")):
                            if (issubclass(imported_module.ExternalParser, Parser)):
                                DataManager.external_files.append(imported_module)
                            else:
                                errors.raise_error(
                                    "DeveloperError", 0, 0,
                                    "The external module '{}'".format(os.path.join(external_directory, file_name))
                                    + " has the class 'ExternalParser' but it is not a subclass of 'parser.Parser'")
                                return False
                        else:
                            errors.raise_error(
                                "DeveloperError", 0, 0,
